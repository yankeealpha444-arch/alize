...
export default function ManualEditor({ projectId = "default" }: ManualEditorProps) {
  ...
  // Resolve current live values: custom override or generated copy
    ...
    <div className="p-4 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button onClick={handleReset} className="flex items-center gap-1.5 text-[10px] text-muted-foreground hover:text-foreground px-2 py-1 rounded border border-border hover:bg-secondary transition-colors">
            <RotateCcw className="w-3 h-3" /> Reset
          </button>
          <button onClick={handleSave} className="flex items-center gap-1.5 text-[10px] font-semibold text-background bg-foreground hover:bg-foreground/90 px-3 py-1 rounded transition-colors">
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center justify-between py-2">
          ...
          <button
            onClick={() => setFields({ ...fields, showTestimonials: !fields.showTestimonials })}
