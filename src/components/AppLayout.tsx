...
const AppLayout = () => {
  ...
  const handleOpenUserView = () => {
    window.open(hashAppUrl(`/p/${pid}`), "_blank", "noopener,noreferrer");
  };
    ...
    <div className="flex h-screen overflow-hidden">
      ...
      <aside className={`${collapsed ? "w-14" : "w-56"} shrink-0 bg-sidebar border-r border-sidebar-border flex flex-col transition-all duration-200`}>
        <div className={`p-4 border-b border-sidebar-border flex items-center ${collapsed ? "justify-center" : "justify-between"}`}>
          {!collapsed && (
            <button onClick={() => navigate("/projects")} className="min-w-0 text-left hover:opacity-80 transition-opacity">
          ...
          {collapsed && (
            <button onClick={() => navigate("/projects")} className="p-1 rounded hover:bg-sidebar-accent text-primary font-bold text-sm transition-colors" title="All projects">
              A
            </button>
          )}
