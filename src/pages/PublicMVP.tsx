...
import { addEmailCapture, addFeedback, addSurveyResponse, getMvpCustomizations, MvpCustomizations } from "@/lib/projectData";
...
export default function PublicMVP() {
  ...
  const [mvpCustom, setMvpCustom] = useState<MvpCustomizations>(() => getMvpCustomizations(projectId));
  useEffect(() => {
    setMvpCustom(getMvpCustomizations(projectId));
  }, [projectId]);
  useEffect(() => {
    const handler = () => setMvpCustom(getMvpCustomizations(projectId));
  ...
  const headline = mvpCustom.headline || copy.headline;
  const subtitle = mvpCustom.subtitle || copy.subtitle;
  const ctaLabel = mvpCustom.ctaText || copy.cta;
  ...
  return (
    <div className="text-foreground min-h-screen bg-background">
      ...
      <div className="bg-secondary/50 border-b border-border px-6 py-1.5 flex items-center justify-between">
        ...
        <button
          type="button"
          onClick={() => { window.location.href = hashAppUrl(`/builder/${projectId}`); }}
      ...
      <Section className="py-24 relative overflow-hidden">
        ...
        <div className="max-w-2xl mx-auto text-center relative">
          ...
          <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight mb-5 leading-[1.1] capitalize">{headline}</h1>
          <p className="text-base text-muted-foreground mb-10 leading-relaxed max-w-lg mx-auto">{subtitle}</p>
      ...
      {includePricing && (
        <Section id="pricing" className="border-t border-border">
          ...
          <p className="text-sm text-muted-foreground text-center mb-10">
            {mvpCustom.pricingCopy || (isToolFirst ? "Try free. Pay only when you need more." : "Start free. Upgrade when you're ready.")}
          </p>
      ...
      <Section className="border-t border-border">
        ...
        <div className="max-w-lg mx-auto space-y-3">
          {[
            { q: `What is ${projectName}?`, a: subtitle },
      ...
      <div className="px-6 py-6 text-center border-t border-border bg-secondary/10">
        ...
        <button
          ...
          className="text-xs text-muted-foreground underline hover:text-foreground mt-2 transition-colors"
        >
          Build with Alizé →
        </button>
      </div>
