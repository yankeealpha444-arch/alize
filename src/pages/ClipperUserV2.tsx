...
export default function ClipperUserV2() {
  ...
  useEffect(() => {
    void trackEvent("page_view", projectId, "clips_v2");
  }, [projectId]);
  ...
  const markSavedBriefly = (clipId: string) => {
    setFeedbackByClipId((prev) => ({
      ...prev,
      [clipId]: { ...(prev[clipId] ?? { sentiment: null, reason: null, saved: false }), saved: true },
    }));
    window.setTimeout(() => {
      setFeedbackByClipId((prev) => ({
        ...prev,
        [clipId]: { ...(prev[clipId] ?? { sentiment: null, reason: null, saved: false }), saved: false },
      }));
    }, 1200);
  };
  ...
  const setSentiment = (clipId: string, sentiment: FeedbackSentiment) => {
    setFeedbackByClipId((prev) => ({
      ...prev,
      [clipId]: {
        ...(prev[clipId] ?? { sentiment: null, reason: null, saved: false }),
        sentiment,
      },
    }));
  };
          ...
          {clips.length === 0 ? (
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {clips.map((clip, idx) => {
                ...
                const feedback = feedbackByClipId[clip.id] ?? { sentiment: null, reason: null, saved: false };
                ...
                return (
                  <article key={clip.id} className="rounded-xl border border-border/60 bg-card p-3">
                    ...
                    <div className="mt-3 flex items-center gap-2">
                      ...
                      {feedback.saved ? <span className="text-xs text-muted-foreground">Saved ✓</span> : null}
                    </div>
