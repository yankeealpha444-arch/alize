...
export default function ClipperAiCeo() {
  ...
  useEffect(() => {
    void trackEvent("page_view", projectId, "clips_ai_ceo");
  }, [projectId]);
  ...
  const handleGenerate = async () => {
    ...
    setIsLoading(true);
    setMessage("");
    void trackEvent("generation_started", projectId, "clips_ai_ceo_generate");
    ...
    try {
      ...
      if (error) {
        ...
        setClips([]);
        void trackEvent("generation_failed", projectId, "clips_ai_ceo_failed");
        return;
      }

      const rows = ((data ?? []) as ClipRow[]).filter((row) => isPlayableUrl(String(row.video_url ?? "")));
      setClips(rows.slice(0, 3));
      if (rows.length === 0) setMessage("No clips yet.");
    } catch {
                ...
                return (
                  <article key={clip.id} className="rounded-xl border border-border/60 bg-card p-3">
                    ...
                    <div className="mt-3 flex gap-2">
                      <button
                        type="button"
                        onClick={() => void trackEvent("clip_feedback_good", projectId, clip.id)}
                        className="inline-flex items-center rounded-md border border-border px-3 py-1 text-xs font-medium hover:bg-secondary"
                      >
                        Good
                      </button>
                      <button
                        type="button"
                        onClick={() => void trackEvent("clip_feedback_bad", projectId, clip.id)}
                        className="inline-flex items-center rounded-md border border-border px-3 py-1 text-xs font-medium hover:bg-secondary"
                      >
                        Bad
                      </button>
