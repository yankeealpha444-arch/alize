...
export default function ClipperUserV4() {
  ...
  useEffect(() => {
    void trackEvent("page_view", projectId, "clips_v4");
  }, [projectId]);
  ...
  useEffect(() => {
    ...
    const loadInitial = async () => {
      try {
        ...
        if (cancelled) return;
        const initialRows = toClipRows(state.clips);
        if (initialRows.length > 0) {
          setClips(initialRows);
          setPreviousPlayableClips(initialRows);
        }
      } catch {
        // Keep initial state quiet.
      }
    };
    void loadInitial();
    return () => {
    ...
    const poll = async () => {
      try {
        ...
        const timedOut =
          ...
          Date.now() - (startedAt ?? 0) > POLL_TIMEOUT_MS;
  ...
  const handleGenerate = async () => {
    ...
    setIsLoading(true);
    setMessage("");
    void trackEvent("generation_started", projectId, "clips_v4_generate");
    ...
    try {
      ...
      setClips(previousPlayableClips);
      setIsLoading(false);
      void trackEvent("generation_failed", projectId, "clips_v4_failed_start");
    }
  };
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <main className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="mb-2 text-xs font-semibold tracking-wide text-amber-700">CLIPPER USER V4</div>
        ...
        <section className="mt-6 max-w-2xl rounded-xl border border-border/60 bg-card p-4">
          ...
          {showProcessing ? <p className="mt-2 text-sm text-muted-foreground">Generating clips for new link...</p> : null}
